...
- [x] Fixed the real root cause of text "disappearing and not coming back": the duplicate + `translateX(-50%)` loop technique assumes the track's start edge aligns left (LTR); on our RTL page the track was right-aligning by default instead, so the two halves never actually lined up mid-cycle. Fixed with `direction: ltr` on `.strip-marquee` itself (Hebrew phrases still render correctly right-to-left within each span — only the ticker's own box layout is forced LTR). Also slowed the animation down (22s→40s desktop, 18s→30s mobile). Considered Swiper.js per the user's suggestion but kept it pure CSS/JS — no need for a carousel library for a decorative ticker.

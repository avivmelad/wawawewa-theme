...
| `Homepage Futuristic Mobile v2.dc.html` | Same as above, mobile viewport (390×844 iOS frame, light `dark={false}` device frame). Verified (2026-07-28) to use **identical hex color values** to the desktop v2 file — no mobile-specific palette differences, only the usual layout/spacing ones already covered by the desktop/mobile pair below. **Note**: its footer section is bigger than the desktop v2 file's — adds a brand blurb + social icons, two link columns, and payment badges not present in `Homepage Futuristic v2.dc.html`. Per explicit decision (2026-07-31) that extra content was deliberately not built — only ask before building it if the user wants that expanded footer. |
